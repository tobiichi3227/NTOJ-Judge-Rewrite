...
from lang.base import BaseLang, CompiledLang, reg_lang
from models import Compiler
from sandbox.sandbox import SandboxParams

@dataclass
class _RISCV64Asm(BaseLang):
    def compile(
        self,
        box,
        ...
        param = SandboxParams(
            exe_path="/usr/bin/riscv64-linux-gnu-gcc",
            args=[
                "-o",
                executable_name,
                *sources,
...
class _AARCH64Asm(BaseLang):
    def compile(
        ...
        param = SandboxParams(
            exe_path="/usr/bin/aarch64-linux-gnu-gcc",
            args=[
                "-o",
                executable_name,
                *sources,
...
reg_lang(
    Compiler.asm_aarch64_with_libc,
    _AARCH64Asm(
        name="asm",
        header_ext="",
        source_ext=".s",
        object_ext=".o",
        executable_ext="",
        allow_thread_count=8,
    ),
)

reg_lang(
    Compiler.asm_riscv64_with_libc,
    _RISCV64Asm(
        name="asm",
        header_ext="",
        source_ext=".s",
